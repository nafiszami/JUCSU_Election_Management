...
$schedule_stmt = $pdo->prepare("SELECT * FROM election_schedule WHERE election_type = 'jucsu'");
$schedule_stmt->execute();
$schedule = $schedule_stmt->fetch(PDO::FETCH_ASSOC);
$voting_date = $schedule['voting_date'] ?? '2026-01-20';
$current_phase = ucwords($schedule['current_phase'] ?? 'voting');

// Fetch total approved candidates
$cand_stmt = $pdo->query("SELECT COUNT(*) FROM candidates WHERE status = 'approved'");
$total_candidates = $cand_stmt->fetchColumn();

// Fetch total verified voters
$voter_stmt = $pdo->query("SELECT COUNT(*) FROM users WHERE role = 'voter' AND is_verified = TRUE");
$total_voters = $voter_stmt->fetchColumn();

// Determine current status display
$status_display = strtoupper($current_phase) . ' PHASE';
if ($current_phase === 'Voting') {
    $status_display = 'VOTING ACTIVE';
} elseif ($current_phase === 'Results') {
    $status_display = 'RESULTS DECLARED';
}
...
<ul class="nav nav-tabs nav-tabs-custom">
    ...
    <li class="nav-item dropdown">
        ...
        <ul class="dropdown-menu" aria-labelledby="statusDropdown">
            <li><a class="dropdown-item" href="#"><?php echo htmlspecialchars($current_phase); ?></a></li>
        </ul>
    </li>
</ul>
    ...
    <div class="col-12">
        <div class="card info-card border-0 shadow-sm">
            <div class="card-body py-4">
                <h3 class="text-center mb-4 fw-bold countdown-title">Days Remaining</h3>
    ...
    <div class="col-md-4 mb-4">
        <div class="card info-card shadow-sm">
            <div class="card-body text-center py-4">
                ...
                <h3 class="text-success fw-bold mb-0"><?php echo htmlspecialchars($status_display); ?></h3>
                <p class="text-muted small mt-2 mb-0">Election is currently in <?php echo htmlspecialchars(strtolower($current_phase)); ?> phase</p>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-4 mb-4">
        <div class="card info-card shadow-sm">
            <div class="card-body text-center py-4">
                ...
                <h3 class="text-primary fw-bold mb-0"><?php echo number_format($total_candidates); ?></h3>
                <p class="text-muted small mt-2 mb-0">Approved candidates</p>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-4 mb-4">
        <div class="card info-card shadow-sm">
            <div class="card-body text-center py-4">
                ...
                <h3 class="text-warning fw-bold mb-0"><?php echo number_format($total_voters); ?></h3>
                <p class="text-muted small mt-2 mb-0">Verified eligible voters</p>
            </div>
        </div>
    </div>
</div>
...
function updateCountdown() {
    const countdownDate = new Date("<?php echo $voting_date; ?>T00:00:00").getTime();
    ...
    if (distance > 0) {
        ...
        document.querySelector('.countdown-title').textContent = 'Election Completed';
    }
