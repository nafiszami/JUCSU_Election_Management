<?php
// public/view_candidates.php
$page_title = "View All Candidates";
require_once 'connection.php';

// Get election type, hall, and position filter from URL
$election_type = $_GET['type'] ?? 'jucsu';
$selected_hall = $_GET['hall'] ?? '';
...
try {
    ...
    $stmt = $pdo->prepare("SELECT hall_name FROM halls WHERE is_active = 1 ORDER BY hall_name");
    $stmt->execute();
    $all_halls = $stmt->fetchAll(PDO::FETCH_COLUMN);

    // Fetch JUCSU candidates
    $stmt = $pdo->prepare("
        SELECT c.id, c.user_id, u.full_name, u.university_id, u.department, 
            p.position_name, c.photo_path, u.hall_name, c.manifesto
        FROM candidates c
...
");
    ...
    // Fetch Hall candidates (all halls)
    if ($election_type === 'hall' && !empty($selected_hall)) {
        $stmt = $pdo->prepare("
            SELECT c.id, c.user_id, u.full_name, u.university_id, u.department, p.position_name, c.photo_path, c.hall_name, c.manifesto
            FROM candidates c
            JOIN users u ON c.user_id = u.id
            JOIN positions p ON c.position_id = p.id
            ...
            ORDER BY p.position_order, u.full_name
        ");
        $stmt->execute([$selected_hall]);
        $hall_candidates = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // Fetch hall positions
        $stmt = $pdo->prepare("
            SELECT position_name 
            FROM positions 
...
<div class="candidates-header">
    <div class="container text-center">
        <h1><i class="bi bi-people-fill"></i> All Election Candidates</h1>
        <p>Browse approved candidates for JUCSU and Hall Union elections</p>
    </div>
</div>
    ...
    <?php else: ?>
        ...
        <div class="election-toggle">
            ...
            <a href="?type=hall#mainContent" class="toggle-btn <?php echo $election_type === 'hall' ? 'active' : ''; ?>">
                ...
                <span>Hall Union Elections</span>
            </a>
        </div>

        <!-- Hall Selector (only for hall elections) -->
        <?php if ($election_type === 'hall'): ?>
            <div class="hall-selector">
                <h4><i class="bi bi-building"></i> Select a Hall</h4>
                <select class="hall-select" onchange="window.location.href='?type=hall&hall=' + this.value + '#mainContent'">
                    <option value="">-- Choose a Hall --</option>
                    <?php foreach ($all_halls as $hall): ?>
                        <option value="<?php echo urlencode($hall); ?>" <?php echo $selected_hall === $hall ? 'selected' : ''; ?>>
        ...
        <!-- Statistics Bar -->
        <?php if ($election_type === 'jucsu' || ($election_type === 'hall' && !empty($selected_hall))): ?>
            <div class="stats-bar">
                <div class="stats-grid">
                    <div class="stat-item">
                        <div class="stat-number"><?php echo count($current_candidates); ?></div>
                        <div class="stat-label">Total Candidates</div>
                    </div>
                    <div class="stat-item">
                        <div class="stat-number"><?php echo count($current_positions); ?></div>
                    ...
                    <div class="stat-item">
                        <div class="stat-number">
                            ...
                            if ($election_type === 'jucsu') {
                                ...
                                echo htmlspecialchars($selected_hall);
                            }
                            ?>
                        </div>
        ...
        <div class="row" id="mainContent">
            <?php if ($election_type === 'hall' && empty($selected_hall)): ?>
                <div class="col-12">
                    <div class="empty-state">
                        <div class="empty-icon"><i class="bi bi-building"></i></div>
                        <h3 class="empty-title">Select a Hall</h3>
                        <p class="empty-text">
                            Please select a hall from the dropdown above to view its election candidates.
                        </p>
                    </div>
                </div>
            <?php else: ?>
                        ...
                        <ul class="position-list">
                            <?php foreach ($current_positions as $position): ?>
                                <li class="position-item">
                                    <?php 
                                    $url = "?type={$election_type}&position=" . urlencode($position);
                                    if ($election_type === 'hall' && !empty($selected_hall)) {
                                        $url .= "&hall=" . urlencode($selected_hall);
                                    }
                                    $url .= "#mainContent";
                                    ?>
                                    <a href="<?php echo $url; ?>" 
                                        ...
                                        <div class="candidate-info">
                                            ...
                                            <div class="candidate-details">
                                                ...
                                                <div class="detail-item">
                                                    ...
                                                    <span><strong>Hall:</strong> <?php echo htmlspecialchars($candidate['hall_name'] ?? 'N/A'); ?></span>
                                                </div>
                                            </div>
